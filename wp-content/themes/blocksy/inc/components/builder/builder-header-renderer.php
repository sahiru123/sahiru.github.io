...
class Blocksy_Header_Builder_Render extends Blocksy_Builder_Render {
	...
	public function render_row($row, $args = []) {
		...
		if (
			...
			in_array(
				'search-input',
				$this->get_placement_by($row, 'start')['items']
			)
		) {
			$attr['data-search-box'] = 'left';
		}
		...
		if (
			...
			in_array(
				'search-input',
				$this->get_placement_by($row, 'end')['items']
			)
		) {
			$attr['data-search-box'] = 'right';
		}
		...
		if (
			...
			in_array(
				'search-input',
				$this->get_placement_by($row, 'middle')['items']
			)
		) {
			$attr['data-search-box'] = 'center';
		}
