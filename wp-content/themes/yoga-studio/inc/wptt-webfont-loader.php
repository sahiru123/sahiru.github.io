...
if ( ! class_exists( 'Yoga_Studio_WPTT_WebFont_Loader' ) ) {
	...
	class Yoga_Studio_WPTT_WebFont_Loader {
		...
		public function get_remote_url_contents() {
			...
			$yoga_studio_contents = wp_remote_retrieve_body( $response );

			return $yoga_studio_contents;
		}
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $yoga_studio_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $yoga_studio_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$yoga_studio_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$yoga_studio_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$yoga_studio_font_family = trim( str_replace( array( "'", ';' ), '', $yoga_studio_font_family ) );
					$yoga_studio_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $yoga_studio_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $yoga_studio_font_family ] ) ) {
					$result[ $yoga_studio_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$yoga_studio_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$yoga_studio_font_family_url = $this->get_absolute_path( $yoga_studio_font_family_url );

					$result[ $yoga_studio_font_family ][] = $yoga_studio_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $yoga_studio_font_family ] = array_flip( array_flip( $result[ $yoga_studio_font_family ] ) );
			}
			return $result;
		}
