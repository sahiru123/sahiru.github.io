...
class Yoga_Customizer_Notify_Section extends WP_Customize_Section {
	
	public $type = 'yoga-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'yoga_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'yoga_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		$yoga_customizer_notify_show_recommended_actions = get_option( 'yoga_customizer_notify_show' );
		foreach ( $yoga_customizer_notify_recommended_actions as $key => $yoga_lite_customizer_notify_recommended_action ) {
			if ( $yoga_customizer_notify_show_recommended_actions[ $yoga_lite_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $yoga_lite_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$yoga_lite_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $yoga_lite_customizer_notify_recommended_action['plugin_slug'] ) ) {
				$active = $this->check_active( $yoga_customizer_notify_recommended_action['plugin_slug'] );
				$yoga_lite_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $yoga_lite_customizer_notify_recommended_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$yoga_lite_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$yoga_lite_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$yoga_lite_customizer_notify_recommended_action['button_class'] = 'install-now button';
						$yoga_lite_customizer_notify_recommended_action['button_label'] = $install_button_label;
						break;
					case 'activate':
						$yoga_lite_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
						$yoga_lite_customizer_notify_recommended_action['button_label'] = $activate_button_label;
						break;
					case 'deactivate':
						$yoga_lite_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
						$yoga_lite_customizer_notify_recommended_action['button_label'] = $deactivate_button_label;
						break;
				}
			}
			$formatted_array[] = $yoga_lite_customizer_notify_recommended_action;
		}// End foreach().

		$customize_plugins = array();

		$yoga_lite_customizer_notify_show_recommended_plugins = get_option( 'yoga_customizer_notify_show_recommended_plugins' );

		foreach ( $yoga_customizer_notify_recommended_plugins as $slug => $plugin_opt ) {

			if ( ! $plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $yoga_lite_customizer_notify_show_recommended_plugins[ $slug ] ) && $yoga_lite_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
			...
			if ( ! empty( $plugin_opt['description'] ) ) {
				...
				$ti_customizer_notify_recommended_plugin['description'] = $info->yoga_description;
			}
		...
		$json['total_actions']       = count( $yoga_customizer_notify_recommended_actions );
		...
		<# if( data.recommended_actions.length > 0 || data.recommended_plugins.length > 0 ){ #>
			<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
				...
				<h3 class="accordion-section-title">
					...
					<# if( data.recommended_actions.length > 0 ){ #>
						<span class="yoga-customizer-plugin-notify-actions-count">
							<span class="current-index">{{ data.recommended_actions[0].index }}</span>
							{{ data.total_actions }}
						</span>
					<# } #>
				</h3>
				<div class="yoga-theme-recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_actions.length > 0 ){ #>
						<# for (action in data.recommended_actions) { #>
							<div class="yoga-recommeded-actions-container epsilon-required-actions" data-index="{{ data.recommended_actions[action].index }}">
								<# if( !data.recommended_actions[action].check ){ #>
									<div class="yoga-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_actions[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no yoga-customizer-notify-dismiss-recommended-action" id="{{ data.recommended_actions[action].id }}"></span>
					...
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="yoga-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="yoga-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_plugins[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no yoga-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>
